...
function twentytwenty_register_scripts() {
	...
	wp_enqueue_script('main', get_template_directory_uri() . '/assets/js/main.js', array('jquery'), '0.0.1', true);
  wp_localize_script('main', 'dx', array('ajaxurl' => admin_url('admin-ajax.php')) );
...
//    print_r($ajaxPosts->get_queried_object()->name);

    while($ajaxPosts->have_posts()) : $ajaxPosts->the_post();
      get_template_part('/template-parts/event');
//      $termCity = get_the_terms(get_the_ID(), "city");
//      $termC = array_shift( $termCity );
//      $termMonth = get_the_terms(get_the_ID(), "month");
//      $termM = array_shift( $termMonth );
//      $termYaer = get_the_terms(get_the_ID(), "years");
//      $termY = array_shift( $termYaer );
//      $response .= '<a href="' . get_the_permalink() . '" class="contentItem"><div class="imgWrap"><img src="' . get_the_post_thumbnail_url() . '" alt=""></div><h2>'. get_the_title() . '</h2><div class="metaBlock"><span class="city">Город: ' . $termC->name . '</span><span class="month">Месяц: ' . $termM->name . '</span><span class="year">Год: ' . $termY->name . '</span></div></a>';
    endwhile;
  } else{
    echo 'Ничего не найдено';
  }

//  wp_send_json(array('status'=>'ok', 'response' => $response, 'cities' => $cities));
...
add_action('wp_ajax_all_events', 'all_events');
add_action('wp_ajax_nopriv_all_events', 'all_events');